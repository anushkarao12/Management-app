...
export function Sidebar({ currentPage, onNavigate, isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <aside className={cn(
      'fixed inset-y-0 left-0 z-40 w-56 bg-white dark:bg-neutral-950 border-r border-neutral-200 dark:border-neutral-800 flex flex-col transition-transform md:translate-x-0',
      isOpen ? 'translate-x-0' : '-translate-x-full'
    )}>
      {isOpen && (
        <button onClick={onToggle} className="absolute top-3 right-3 md:hidden p-1 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800">
          <X size={18} />
        </button>
      )}
...
export function MobileMenuButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="md:hidden p-2 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-800">
      <Menu size={20} />
    </button>
  );
}
